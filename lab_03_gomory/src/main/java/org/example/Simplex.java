...
public class Simplex {
    ...
    //симплекс-метод
    public static void makeSimplex() {
        int mainCol, mainRow;
        while (!isEnd()) {
            ...
            Matrix.printMatrix(Gomory.table,Gomory.basis);
        }
    }
    ...
    public static int findMainRow(int mainCol) {
        ...
        for (int i = 0; i < Gomory.m - 1; i++) {
            if ((Gomory.table[i][mainCol] > 0) &&
                    ((Gomory.table[i][0] / Gomory.table[i][mainCol]) > 0) &&
                    ((Gomory.table[i][0] / Gomory.table[i][mainCol]) < (Gomory.table[mainRow][0] / Gomory.table[mainRow][mainCol])))
                mainRow = i;
        }
        return mainRow;
    }
