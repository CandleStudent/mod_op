...
class FordFulkerson {
        ...
        while (!queue.isEmpty()) {
            ...
            for (int v = 0; v < V; v++) {
                if (!visited[v] && rGraph[u][v] > 0) {
                    ...
                    parent[v] = u;
                    // Обновляем родителя для вершины v (это означает, что в вершину v мы попадаем из u)
    ...
    static int fordFulkerson(int[][] graph, int s, int t) {
        ...
        // bfs меняет массив parent. После этого метода он становится устроен следующим образом:
        // parent[i] = j ==> в вершину i мы попадаем из j
        // т.о. массив Parent задает путь от истока до стока
        while (bfs(graph, s, t, parent)) {
            ...
            for (v = t; v != s; v = parent[v]) {
                ...
                u = parent[v];
                output.add(u+1);
                path_flow = Math.min(path_flow, graph[u][v]); // минимальная потому, что поток в пути не может быть больше минимальной пропускной способности узла, входящего в маршрут
            }
    ...
    public static void main(String[] args) {
        ...
        // Вызываем метод для нахождения максимального потока от источника (0) до стока (6)
